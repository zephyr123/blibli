...
if __name__ == '__main__':
    ...
    api_url = "http:/xxx:8443/api"  # xxx.xxx.xxx部分自行更换为harbor首页url
    # Login ,change username and password
    login = HTTPBasicAuth('', '')   # 自行更改用户名，密码
    # 需要排除的项目组，自行根据情况更改，或为空
    exclude = ['library']    # 自行更改需排除项目组，也可以删除为空; 例:exclude = ['k8s', 'basic', 'library']
    # 仓库下版本过多，需保留的最近版本数量
    keep_num = 15
    # 启动Start the engine
